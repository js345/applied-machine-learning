...
wmod<-cv.glmnet(wmat, y, alpha=1)
plot(wmod,main="lasso")
# 100 * 100 grid
xmin<-min(xmat[,1])
xmax<-max(xmat[,1])
ymin<-min(xmat[,2])
...
image.plot(xvec, yvec, zmat ,xlab='East_UTM', ylab='North_UTM', col=heat.colors(20,alpha = 1), useRaster=TRUE, legend.only=FALSE, main="lasso")
